...
class widget_socialicons extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		foreach ( $instance as $ins => $value ) {
			if ( in_array( $ins, array( 'Facebook', 'Twitter', 'Instagram', 'Google', 'SnapChat', 'Tumblr', 'Linkedin' ) ) && $value ) {
				if ( $style === 'style1' ) {
					$count++;
				}
			}
	...
	function form( $instance ) {
		...
		?>
			...
			<p>
				...
				<input id="<?php echo esc_attr( $this->get_field_id( 'style1' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" type="radio" value="style1"
										<?php
										if ( $instance['style'] === 'style1' || ! $instance['style'] ) {
											echo 'checked="checked"'; }
										?>
				 />Style 1
				</label><br>
				<label for="<?php echo esc_attr( $this->get_field_id( 'style2' ) ); ?>">
				<input id="<?php echo esc_attr( $this->get_field_id( 'style2' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" type="radio" value="style2"
										<?php
			...
			<p>
				...
				<input id="<?php echo esc_attr( $this->get_field_id( 'Linkedin' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'linkedin' ) ); ?>" value="<?php echo esc_attr( $instance['Linkedin'] ); ?>" class="widefat" />
			</p>
		<?php
	}
}
