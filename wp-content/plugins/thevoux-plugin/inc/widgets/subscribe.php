...
class thb_subscribe_widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			...
			<input id="<?php echo esc_attr( $this->get_field_id( 'style1' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" type="radio" value="dark-text" 
								  <?php
									if ( $style === 'dark-text' || ! $style ) {
										echo 'checked="checked"'; }
									?>
			 /> <?php esc_html_e( 'Dark Text', 'thevoux' ); ?>
			</label><br>
			<label for="<?php echo esc_attr( $this->get_field_id( 'style2' ) ); ?>">
			<input id="<?php echo esc_attr( $this->get_field_id( 'style2' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" type="radio" value="light-text" 
								  <?php
									if ( $style === 'light-text' ) {
										echo 'checked="checked"'; }
									?>
			 /> <?php esc_html_e( 'Light Text', 'thevoux' ); ?>
			</label>
		</p>
