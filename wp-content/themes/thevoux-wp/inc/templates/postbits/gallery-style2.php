...
foreach ( $post_gallery_photos_arr as $attachment ) {
	...
	<figure class="thb-gallery-item">
		...
		<div class="thb-gallery-nav">
			<span class="arrow to_top
			<?php
			if ( 1 == $i ) {
				?>
				 visually-hidden<?php } ?>"><?php get_template_part( 'assets/svg/arrow_prev.svg' ); ?></span>
			<div class="count"><?php echo esc_html( $i ); ?> <em>/</em> <?php echo esc_html( $count ); ?></div>
			<span class="arrow to_bottom
			<?php
			if ( $count == $i ) {
				?>
				 visually-hidden<?php } ?>"><?php get_template_part( 'assets/svg/arrow_next.svg' ); ?></span>
		</div>
		</div>
