	...
	<div class="thb-hotspot-container <?php echo esc_attr( $thb_tooltip_function ); ?>">
		...
		<?php foreach ( $spots as $key => $spot ) { ?>
			...
			<div class="thb-hotspot-content <?php echo esc_attr( $thb_pulsate ); ?>"><?php echo esc_attr( $key + 1 ); ?></div>
				...
				<?php
				if ( $spot->Product && thb_wc_supported() ) {
					?>
					thb-product-popup<?php } ?>">
				<?php
				if ( $spot->Product && thb_wc_supported() ) {
					$product_id = is_array( $spot->Product ) ? $spot->Product[0][0] : $spot->Product;
					...
					?>
					<?php
					if ( ! $product_loop->found_posts ) {
						?>
						<p><?php esc_html_e( 'Product Not Found', 'thevoux' ); ?></p><?php } ?>
				<?php } else { ?>
					<?php
					if ( $spot->Title ) {
						?>
						<h6><?php echo wp_kses_post( $spot->Title ); ?></h6><?php } ?>
					<?php
					if ( $spot->Message ) {
						?>
						<p><?php echo wp_kses_post( $spot->Message ); ?></p><?php } ?>
				<?php } ?>
				</div>
			</div>
			</div>
...
add_shortcode( 'thb_hotspots', 'thb_hotspots' );
