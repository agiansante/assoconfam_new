...
class THB_Social_Links {
	...
	public function thb_get_medium( $username ) {
		...
		// Check user
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter a Medium User.', 'thevoux' );
			return $result;
		}

		// Get Count
		$data = $this->thb_remote_get(
			array(
				'name'     => 'medium',
				'username' => $username,
	...
	public function thb_get_youtube( $username ) {
		...
		// Check username
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter an YouTube User.', 'thevoux' );
			return $result;
		}
		...
		if ( isset( $data->error->message ) ) {
			...
			// API Error
			$result['error'] = esc_html__( 'Please check your username or channel IDs.', 'thevoux' );
	...
	public function thb_get_twitter( $username ) {
		...
		// Check username
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter a Twitter User.', 'thevoux' );
			return $result;
		}
		...
		if ( ! $data ) {
			...
			if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
				...
				if ( ! isset( $data->errors ) ) {
					$result['error'] = esc_html__( 'Not Available', 'thevoux' );
				}
			} else {
				$data = json_decode( wp_remote_retrieve_body( $response ) );
			}
	...
	public function thb_get_pinterest( $username ) {
		...
		// Check username
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter a pinterest user name.', 'thevoux' );
			return $result;
		}
	...
	public function thb_get_facebook( $username ) {
		...
		// Check username
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter a Facebook user name.', 'thevoux' );
		...
		// Set Count
		$fb_result = preg_match( '/<div[^>]*?>([^>]*?)like/s', $data, $facebook_data );
		...
		if ( $fb_result ) {
			...
			// Live Count.
			$result['count'] = abs( filter_var( wp_strip_all_tags( $fb_data_json ), FILTER_SANITIZE_NUMBER_INT ) );
		} else {
	...
	public function thb_get_instagram( $username ) {
		...
		// Check Username
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter an Instagram Username.', 'thevoux' );
			return $result;
		}
	...
	public function thb_get_vimeo( $username ) {
		...
		// Check username
		if ( ! $username ) {
			$result['error'] = esc_html__( 'Please enter Vimeo Channel ID.', 'thevoux' );
			return $result;
		}
