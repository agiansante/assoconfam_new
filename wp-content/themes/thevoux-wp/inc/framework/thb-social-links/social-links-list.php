...
function thb_get_social_list( $labels = true, $counts = true, $layout = 'thb-social-vertical', $style = 'fill-color' ) {
	...
	<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
		...
		<div class="thb-social-link-wrap">
			<a href="<?php echo esc_url( $url ); ?>" rel="noreferrer" target="_blank" class="thb-social-link social-link-<?php echo esc_attr( $network ); ?>">
				<div class="thb-social-label-container">
					<div class="thb-social-icon-container"><i class="fa fa-<?php echo esc_attr( $network ); ?>"></i></div>
					<?php
					if ( $labels ) {
						?>
						<div class="thb-social-label"><?php echo esc_html( $value['label'] ); ?></div><?php } ?>
				</div>
