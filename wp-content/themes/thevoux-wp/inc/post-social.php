		...
		switch ( $button ) {
			...
			case 'twitter':
				$sharing_accounts['twitter'] = array(
					'slug'  => 'twitter',
					'url'   => esc_url( 'https://twitter.com/intent/tweet?text=' . $post_title . '&via=' . $twitter_user . '&url=' . $share_url ),
					'icon'  => 'fa fa-twitter',
...
function thb_social_article_detail( $id = false, $fixed = false, $class = false ) {
	...
	<aside class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
		...
		foreach ( $sharing_buttons as $button ) {
			...
			?>
			<a href="<?php echo esc_attr( $button['url'] ); ?>" rel="noreferrer" class="boxed-icon social <?php echo esc_attr( $button['slug'] ); ?> <?php echo esc_attr( $sharing_style ); ?>">
...
function thb_social_article( $id ) {
	...
	<?php if ( ! empty( $sharing_buttons ) ) { ?>
		...
		<?php foreach ( $sharing_buttons as $button ) { ?>
			<a href="<?php echo esc_attr( $button['url'] ); ?>" rel="noreferrer" class="boxed-icon fill social <?php echo esc_attr( $button['slug'] ); ?>">
				<i class="<?php echo esc_attr( $button['icon'] ); ?>"></i>
			</a>
		<?php } ?>
	<?php } ?>
