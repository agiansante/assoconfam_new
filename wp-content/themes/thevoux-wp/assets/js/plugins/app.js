				...
				quick_search.on('click touchend', function(e) {
					...
					if (e.target.classList.contains('quick_search') || e.target.classList.contains('search_icon')) {
						...
						if (_this.hasClass('active') ) {
							setTimeout(function(){ _this.find('.search-field').get(0).focus(); }, 1000);
						}
					...
					items.each(function() {
						...
						arrow_top.on('click', function() {
							if (i > 0) {
								...
								gsap.to(win, { duration: 1, scrollTo: { y:pos, autoKill:false } });
							}
							return false;
						});
			...
			init: function() {
                ...
                count: count,
								...
								if ( data === '' || data === 'undefined' || data === 'No More Posts' || data === 'No $args array created') {
									...
									gsap.fromTo(animate, { display: 'none', autoAlpha: 0, y:20 }, { duration: 0.25, display: 'block', y: 0, autoAlpha:1, stagger: 0.15 });
	                ...
	                count: count,
								...
								success : function(data) {
									...
									if( data === '' || data === 'undefined' || data === 'No More Posts' || data === 'No $args array created') {
										...
										$(d).appendTo(_this).hide();
										gsap.fromTo($(d), { duration: 0.25, display: 'none', opacity: 0, y:20, stagger: 0.15 }, { display: 'block', y: 0, opacity:1, onComplete: function() {
											if (l >= count) {
		...
		writeFirst: {
			...
			init: function() {
				...
				container.on('click', function() {
					...
					gsap.to(window, { duration: win.height() / 500, scrollTo: { y:pos, autoKill:false } });
					return false;
				});
		...
		toTop: {
			...
			control: function() {
				...
				if (win.scrollTop() > 300) {
					gsap.to(container, { duration: 0.2, autoAlpha:1 });
				} else {
					TweenMax.to(container, { duration: 0.2, autoAlpha:0 });
				}
			}
		},
		quantity: {
