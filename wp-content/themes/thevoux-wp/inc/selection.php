...
.post.carousel-style12:hover {
	background: rgba(<?php echo thb_hex2rgb( $accent_color ); ?>, 0.1);
}
blockquote:before,
blockquote:after {
	background: rgba(<?php echo thb_hex2rgb( $accent_color ); ?>, 0.2);
...
<?php } ?>
	<?php if ( $subfooter_link_color = ot_get_option( 'subfooter_link_color' ) ) { ?>
		...
		<?php
		if ( 'dark' === ot_get_option( 'subfooter_color' ) ) {
			thb_linkcoloroutput( $subfooter_link_color, '#subfooter.dark' ); }
		?>
<?php } ?>
...
#subfooter .footer-logo-holder .logoimg[src$=".svg"] {
	max-height: 100%;
	height: <?php thb_measurementoutput( $subfooter_logo_height ); ?>;
}
	<?php } ?>
